...
- [x] IMS-062 · Schema & meta validation (`docs/schema-validation-report.md`) — (a) Rich Results Test: JSON-LD extracted from each production page's rendered `<script type="application/ld+json">` blocks into a minimal standalone wrapper doc, submitted via the tool's "code" tab (localhost isn't reachable by the "url" tab). Same 3 representative slugs as IMS-061. 0 schema errors on all 3, all three target types (`Service`, `FAQPage`, `BreadcrumbList`) present and valid — 3 valid items detected per slug (Breadcrumbs, Local businesses, Organization). Only issues surfaced are 3 identical non-critical optional-field notices (telephone/priceRange/address) on the sitewide `ProfessionalService` schema, pre-existing and out of this ticket's scope. (b) `npm run check:meta` (new script, `lib/industries/check-meta-uniqueness.ts`) crawls all 11 pages and asserts unique titles/descriptions — 0 duplicates, 0 missing. Both AC parts pass.
- [x] IMS-063 · Cross-device manual pass (`docs/qa-device-matrix.md`) — no physical iOS Safari/Android Chrome/macOS Safari devices in this environment, so Doc 1's 5-target matrix is approximated with Playwright's own rendering engines (chromium/firefox/webkit + device-preset UA/touch options as Android Chrome and iOS Safari proxies), explicitly flagged as engine-based coverage, not physical-device verification, in `docs/qa-device-matrix.md`. New tooling added outside Doc 1's file map (flagged deviation, rule 7): `playwright.device-matrix.config.ts` (separate from the locked `playwright.config.ts`, kept out of the `test:e2e` CI gate) and `tests/qa/device-matrix.spec.ts`. Found + fixed one real defect: horizontal scroll at 360px on `/industries/[slug]`, root-caused to `SampleFileChip`'s unconstrained button width (`components/industry/demo/SampleFileChip.tsx`, added `max-w-full flex-wrap`), confirmed via a direct DOM overflow sweep and re-verified with 0 overflowing elements after the fix. 76 passed / 10 skipped (capability-gated: tap only applies to the two touch-proxy projects, drag only to the three non-touch desktop projects) / 0 failed. One pre-existing, out-of-scope console warning (`THREE.Clock` deprecation on desktop-firefox) matches the same sitewide `NoiseField`/`three` dependency already root-caused and documented in IMS-061's `docs/lighthouse-report.md` — not a new or Industries-specific issue.
- [x] IMS-064 · Launch (`docs/launch.md`) — **STOPPED at preview per binding user override of Doc 1's literal text; production, GSC sitemap, and `/dev/tokens`/`/dev/demo` deletion all withheld pending separate explicit approval.** Mid-ticket platform pivot (user-directed, flagged deviation): Vercel → **Cloudflare Workers**, via `@opennextjs/cloudflare` + `wrangler`, scoped to preview only — production stays on Netlify, untouched. Next.js bumped 16.2.4 → 16.2.11 (user-directed) and verified regression-free with a 51/51 local e2e pass before any Cloudflare work began. New tooling outside Doc 1's file map (flagged deviation, rule 7): `wrangler.jsonc`, `open-next.config.ts`, `public/_headers`, `cf:build`/`cf:preview`/`cf:deploy` npm scripts, and an additive `PLAYWRIGHT_BASE_URL` override in `playwright.config.ts` enabling the locked e2e suite to target a live remote deploy. Missing `workers.dev` subdomain registered via direct Cloudflare API call (CLI-first, no dashboard escalation) rather than asking the user to click through onboarding. **Live preview: https://ims-consultancy-preview.ims-consultancy.workers.dev.** Investigated two transient cold-start 404s on first deploy (`accountancy-firms`, `private-healthcare-groups`) via prerender-manifest/cache/bundle/`wrangler tail` inspection — confirmed a cache-population race on a fresh Workers deploy, not a defect; stable on every re-check. All gates re-run against the live preview (not localhost): route sweep 12/12 pass; IMS-060 e2e 51/51 pass (one earlier mid-session `THREE.Clock` console-warning flake on `b2b-saas`, already root-caused in IMS-061/IMS-063 as a pre-existing sitewide `NoiseField` issue, reproduced clean on the final run); IMS-062 meta-uniqueness clean (11/11 unique); IMS-061 Lighthouse re-run on the same 4 targets scores 98–100 across all four categories on every target (up from the IMS-061 localhost baseline of hub 89 / slugs 84), verified as genuine (no `runtimeError`, matching requested/final URLs, real network/timing data) rather than a false positive from a failed load. IMS-063's device matrix was not re-run against the preview specifically (DOM is unchanged by hosting platform); flagged as a leaning, not a finalized, decision. Full detail in `docs/launch.md`.
